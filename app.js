...
async function loadTasks(R) {
  ...
  const mainUnits = (await query(`
    ...
    unit_actual AS (
      SELECT task, match_id, code, half, side, SUM(CAST(REPLACE(actual_time_taken, ',', '') AS NUMERIC)) AS unit_total
      FROM anchored
  ...
  const playersUnits = (await query(`
    ...
    SELECT task, match_id, code, SUM(CAST(REPLACE(actual_time_taken, ',', '') AS NUMERIC)) AS actual
    FROM anchored
    WHERE julianday(review_started) - julianday(anchor) <= 1.0
    GROUP BY task, match_id, code
  `, [start, end, ...ef.args])).rows;
  ...
  const bcUnits = (await query(`
    ...
    unit_actual AS (
      SELECT task, match_id, code, half, SUM(CAST(REPLACE(actual_time_taken, ',', '') AS NUMERIC)) AS unit_total
      FROM anchored
      WHERE julianday(review_started) - julianday(anchor) <= 1.0
  ...
  const trendMain = (await query(`
    ...
    unit_actual AS (
      SELECT task, bucket, match_id, code, half, side, SUM(CAST(REPLACE(actual_time_taken, ',', '') AS NUMERIC)) AS unit_total
      FROM anchored
      WHERE julianday(review_started) - julianday(anchor) <= 1.0
  ...
  const trendPlayers = (await query(`
    ...
    SELECT task, bucket, match_id, code, SUM(CAST(REPLACE(actual_time_taken, ',', '') AS NUMERIC)) AS actual
    FROM anchored
    WHERE julianday(review_started) - julianday(anchor) <= 1.0
    GROUP BY task, bucket, match_id, code
  `, [start, end, ...ef.args])).rows;
  ...
  const trendBc = (await query(`
    ...
    unit_actual AS (
      SELECT task, bucket, match_id, code, half, SUM(CAST(REPLACE(actual_time_taken, ',', '') AS NUMERIC)) AS unit_total
      FROM anchored
      WHERE julianday(review_started) - julianday(anchor) <= 1.0
...
async function loadHours(R) {
  ...
  const { rows } = await query(`
    ...
    SELECT a.code AS code, a.reviewer_name AS reviewer_name, a.team AS team,
           ...
           SUM(CAST(REPLACE(dl.actual_time_taken, ',', '') AS NUMERIC))  AS actual,
           SUM(CAST(REPLACE(dl.total_break_time, ',', '') AS NUMERIC))   AS break_time,
...
async function loadMatchesDaily() {
  ...
  const [homeRes, awayRes, slaRes, matchIdRes] = await Promise.all([
    query(`SELECT DISTINCT priority_sb_home AS v FROM quality_delivery_time WHERE priority_sb_home IS NOT NULL AND priority_sb_home <> '' ORDER BY 1`),
    query(`SELECT DISTINCT priority_sb_away AS v FROM quality_delivery_time WHERE priority_sb_away IS NOT NULL AND priority_sb_away <> '' ORDER BY 1`),
    query(`SELECT DISTINCT game_sla AS v FROM quality_delivery_time WHERE game_sla IS NOT NULL AND game_sla <> '' ORDER BY 1`),
    query(`SELECT DISTINCT matchid AS v FROM quality_delivery_time WHERE matchid IS NOT NULL AND matchid <> '' ORDER BY 1`),
  ]);
  const homeOpts = homeRes.rows.map(r => r.v);
  const awayOpts = awayRes.rows.map(r => r.v);
  const slaOpts  = slaRes.rows.map(r => r.v);
